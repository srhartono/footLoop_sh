...
","NA");
			...
			my $Rscript = "
.libPaths( c(\"/home/mitochi/R/x86_64-pc-linux-gnu-library/3.6\",
\"/home/mitochi/src/R-3.6.1/library\",\"/home/mitochi/src/R-3.6.1/lib\"))
library(png)
library(Cairo)
library(labeling)
library(ggplot2)
...
";
			...
			if (not -e $currFile or (-e $currFile and linecount($currFile) <= 5)) {
				...
				close $incurrFile;
				close $outcurrFileID;
				my ($R) = Rscript($currFile, $bedFile, $curr_cluster_file, $totpeak, $totnopk, $pngout, $pdfout, $lenpdfout, $resDir, $outLog);
...
#					print "\n\n----------------- $LCY$currFile$N IS A PEAK FILE R = $currFile.PNG.R -------------- \n\n";
	...
	foreach my $outRscriptPNG (sort keys %Rscripts) {
		...
		if (($opt_r == 2) or ($opt_r == 1 and $runR == 1)) {
			...
			#next if $runType !~ /PEAK/;# and $runType ne "NOPK_C";
...
sub Rscript {
	my ($currFile, $bedFile, $curr_cluster_file, $totpeak, $totnopk, $pngout, $pdfout, $lenpdfout, $resDir, $outLog) = @_;
#	LOG($outLog, "\n\n$curr_cluster_file.finalclust\n\n");
	my $currFileID = $currFile . ".id";
	my $R;
...
for (i in 1:length(clust\$id2)) {
	if (length(grep(\",\",clust\$id2[i])) > 0) {
	} else if (as.numeric(clust\$id2[i]) > 40) {
		clust\$id2[i] = 40
	}
}
...
#total
clust2temp = as.data.frame(summary(as.factor(clust\$clust)))
clust2temp\$clust = rownames(clust2temp)
colnames(clust2temp) = c(\"totalclust\",\"clust\")
#clust2temp\$totalclust = paste(\"total=\",clust2temp\$totalclust,sep=\"\")
clust2 = merge(clust2, clust2temp, by=\"clust\")


clust2\$realclust = paste(\"\",clust2\$id2,sep=\"\")
clust2\$length = \"multipeak\"
clust2\$tempclust = \"multipeak\"

for (i in 1:length(clust2\$id2)) {
	if (length(grep(\",\",clust2\$id2[i])) > 0) {
		clust2\$realclust[i] = paste(\"\",gsub(\"^([0-9])[0-9],([0-9])[0-9].*\$\",\"\\\\1,\\\\2\",clust2\$id2[i],perl=T),sep=\"\")
		clust2\$tempclust[i] = \"multipeak\"
		clust2\$length[i] = \"multipeak\"
		clust2\$totalclust[i] = clust2\$totalclust[i] / 2
	} else {
		if (as.numeric(clust2\$id2[i]) == 10) {
			clust2\$length[i] = \"< 250\"
		} else if (as.numeric(clust2\$id2[i]) == 20) {
			clust2\$length[i] = \"250 - 500\"
		} else if (as.numeric(clust2\$id2[i]) == 30) {
			clust2\$length[i] = \"500 - 750\"
		} else {
			clust2\$length[i] = \"> 750\"
		}
		clust2\$realclust[i] = paste(\"\",as.integer(as.numeric(clust2\$id2[i]) / 10),sep=\"\")
		clust2\$clust[i] = as.integer(as.numeric(clust2\$id2[i]) / 10)
		clust2\$tempclust[i] = paste(clust2\$clust[i], \". \", clust2\$length[i],sep=\"\")
	}
}

#for (i in 1:length(clust2\$id2)) {
#	if (length(grep(\",\",clust2\$id2[i])) > 0) {
#		clust2\$realclust[i] = paste(\"clust=\",gsub(\"^([0-9]+)[0-9]?,([0-9]+)[0-9].*\$\",\"\\\\1,\\\\2\",clust2\$id2[i],perl=T),sep=\"\")
#	} else {
#		if (as.numeric(clust2\$id2[i]) %% 10 == 0) {
#			clust2\$length[i] = \"< 250\"
#		} else if (as.numeric(clust2\$id2[i]) %% 10 == 1) {
#			clust2\$length[i] = \"250 - 500\"
#		} else if (as.numeric(clust2\$id2[i]) %% 10 == 1) {
#			clust2\$length[i] = \"500 - 750\"
#		} else {
#			clust2\$length[i] = \"> 750\"
#		}
#		clust2\$length[i] = paste(\"length=\",clust2\$length[i],sep=\"\")
#		clust2\$realclust[i] = paste(\"\",as.integer(as.numeric(clust2\$id2[i]) / 10),sep=\"\")
#		clust2\$clust[i] = as.integer(as.numeric(clust2\$id2[i])/10)
#	}
#}
#length barplot
clust2temp = as.data.frame( aggregate(clust2\$totalclust,by=list(clust2\$tempclust),sum))
colnames(clust2temp) = c(\"length\",\"total\")
clust2temp = clust2temp[grep(\"multipeak\",clust2temp\$length,invert=T),]
if (dim(clust2temp)[1] > 0) {
	clust2temp\$totalperc = as.integer(clust2temp\$total / sum(clust2temp\$total) * 1000+0.5)/10
	totalpeaknomulti = sum(clust2temp\$total)
	pdf(file=\"$pngout.length.pdf\",width=7,height=4)
	pclust2temp = ggplot(clust2temp,aes(length,totalperc)) + geom_bar(aes(fill=length),color=\"black\",stat=\"identity\") + 
		geom_text(aes(group=length,label=paste(totalperc,\"\% (n=\",total,\")\",sep=\"\"),y=totalperc+2.5),size=2) +
		theme_bw() + theme(panel.grid=element_blank(),axis.ticks.x=element_blank(),axis.text.x=element_blank()) +
		ylab(\"Total Peak (%)\") + xlab(\"\") + ggtitle(paste(\"$currFilename\\n(peak=$totpeak; nopk=$totnopk; peak (no multi)=\",totalpeaknomulti,\")\",sep=\"\")) +
	...
	pdf(file=\"$pngout.length.pdf\",width=7,height=4)
	print(plot(x=seq(1,5),y=seq(1,5),main=paste(\"$currFilename\\n(peak=$totpeak; nopk=$totnopk; has no cluster)\",sep=\"\")))
	dev.off()
}


#clust2\$totalclust = paste(\"total=\",clust2\$totalclust,sep=\"\")
#clust2\$length = paste(\"length=\",clust2\$length,sep=\"\")
clust2\$id2 = paste(clust2\$realclust,\". \",clust2\$length,\" (\",clust2\$totalclust,\")\",sep=\"\")
clust2\$id3 = clust2\$realclust
clust2 = subset(clust2,select=c(\"clust\",\"ymin\",\"ymax\",\"xpos0\",\"xpos1\",\"id2\",\"id3\"))
print(clust2)
clust2\$xmin = -100
clust2\$xmax = 140
clust2\$clust = clust2\$clust + 10
write.table(clust2,file=\"$curr_cluster_file.finalclust\",quote=F,row.names=F,col.names=T,sep=\"\\t\")
...
df4\$clust = seq(1,max(df3clust))#df4\$clust
#print(head(df3clust,n=20))
df4\$y = df4\$clust
...
#		 annotate(geom=\"text\",x=0,y=max(dm\$y),hjust=0,vjust=0,label=paste(\"PLOT #\",i,\" (total peak=\",$totpeak,\"; total nopk=\",$totnopk,\")\",sep=\"\"))
	plot_list[[i]] = p
}
";
...
p.png = p +
	 geom_rect(data=clust2,aes(group=as.factor(id3),fill=as.factor(clust),x=-350,y=ymin,xmin=-350,xmax=-100,ymin=ymin,ymax=ymax),color=rgb(0,0,0,0),alpha=1) + #,size=0.5*p.png.scale) + # SIZE
	 geom_rect(data=clust2,aes(group=as.factor(id3),fill=as.factor(clust),x=-100,y=ymin,xmin=-100,xmax=-1,ymin=ymin,ymax=ymax),color=rgb(0,0,0,0),alpha=1) + #,size=0.5*p.png.scale) + # SIZE
	 geom_text(data=clust2,aes(group=as.factor(id3),x=-110,y=(ymin+ymax)/2,label=id2),hjust=1,size=4*p.png.scale) +
	 theme(plot.title = element_text(size = 10*p.png.scale)) +
	 annotate(geom=\"rect\",x=-450,y=1,xmin=-450,ymin=1,xmax=-350,ymax=max(dm\$y),size=1,color=rgb(0,0,0,0),fill=rgb(0,0,0,0),lwd=0.5) +
	 annotate(geom=\"text\",angle=90,x=-370,y=max(dm\$y)/2,size=4*p.png.scale,hjust=0.5,vjust=0.5,label=paste(\"peak=\",$totpeak,\"; nopk=\",$totnopk,\"; peak (no multi)=\",totalpeaknomulti,sep=\"\"))
...
#	 annotate(geom=\"rect\",x=-350,y=1,xmin=-350,ymin=1,xmax=-105,ymax=max(dm\$y),size=1,color=\"black\",fill=rgb(0,0,0,0),lwd=0.5) +
...
p.pdf = p +
	 geom_rect(data=clust2,aes(group=as.factor(id3),fill=as.factor(clust),x=-350,y=ymin,xmin=-350,xmax=-100,ymin=ymin,ymax=ymax),color=rgb(0,0,0,0),alpha=1) + #,size=0.5*p.png.scale) + # SIZE
	 geom_rect(data=clust2,aes(group=as.factor(id3),fill=as.factor(clust),x=-100,y=ymin,xmin=-100,xmax=-1,ymin=ymin,ymax=ymax),color=rgb(0,0,0,0),alpha=1) + #,size=0.5*p.png.scale) + # SIZE
	 geom_text(data=clust2,aes(group=as.factor(id3),x=-110,y=(ymin+ymax)/2,label=id2),hjust=1,size=4*p.pdf.scale) +
	 theme(plot.title = element_text(size = 20*p.pdf.scale)) +
	 annotate(geom=\"rect\",x=-450,y=1,xmin=-450,ymin=1,xmax=-350,ymax=max(dm\$y),size=1,color=rgb(0,0,0,0),fill=rgb(0,0,0,0),lwd=0.5) +
	 annotate(geom=\"text\",angle=90,x=-400,y=max(dm\$y)/2,size=4*p.pdf.scale,hjust=0.5,vjust=0.5,label=paste(\"peak=\",$totpeak,\"; nopk=\",$totnopk,\"; peak (no multi)=\",totalpeaknomulti,sep=\"\"))
...
#	 annotate(geom=\"rect\",x=-350,y=1,xmin=-350,ymin=1,xmax=-105,ymax=max(dm\$y),size=1,color=\"black\",fill=rgb(0,0,0,0),lwd=0.5) +
#	 annotate(geom=\"rect\",x=0,y=0,xmin=0,ymin=0,xmax=max(dm\$variable),ymax=max(dm\$y),size=1,color=\"black\",fill=rgb(0,0,0,0),lwd=0.5)
...
png(type=\"cairo\",file=\"$pngout\",width=totalwidth,height=totalheight)
print(p.png)
dev.off()
#png(type=\"cairo\",file=\"$pngout\",width=totalwidth,height==totalheight)
#if (mynrow == 3) {
...
png(type=\"cairo\",file=\"$pngout\",width=totalwidth,height=totalheight)
print(p.rand.1000.png,ncol=1,nrow=mynrow,heights=totalratio)
...
# PNG
totalheight = (dim(df.rand.100)[1]) * myscale # + 31.25) * myscale
png(pngout_nopk_rand_100,width=totalwidth,height=totalheight)
print(p.rand.100.png)
...
# PDF
currheight = (dim(df.rand.1000)[1]) * myscale / $PDFSCALE # + 31.25) * myscale / $PDFSCALE
currwidth = totalwidth / $PDFSCALE
pdf(\"$pdfout\",width=currwidth,height=currheight)
print(p.rand.1000.pdf)
...
";
	...
	$R->{PDF_nopk_rand_100} = "
currheight = (dim(df.rand.100)[1]) * myscale / $PDFSCALE # + 31.25) * myscale / $PDFSCALE
currwidth = totalwidth / $PDFSCALE
...
#clust2$xpos1 = aggregate(clust$xmax,by=list(clust$clust,clust$id2),max)$x
#colnames(clust2) = c("clust","id2","ymin","ymax","xpos0","xpos1")
#clust2$realclust = paste("clust=",clust2$id2,sep="")
#clust2$length = "len=multipeak"
#for (i in 1:length(clust2$id2)) {
#	if (length(grep(",",clust2$id2[i])) > 0) {
#		clust2$realclust[i] = paste("clust=",gsub("^([0-9])[0-9]+,([0-9])[0-9]+.*$","\\1,\\2",
#clust2$id2[i],perl=T),sep="")
#	} else {
#		if (as.numeric(clust2$id2[i]) %% 10 == 0) {
#			clust2$length[i] = 0
#		} else if (as.numeric(clust2$id2[i]) %% 10 == 1) {
...
#		}
#		clust2$length[i] = paste("length=",clust2$length[i],sep="")
#		clust2$realclust[i] = paste("clust=",as.integer(as.numeric(clust2$id2[i]) / 10),sep="")
#	}
#}
#clust2temp = as.data.frame(summary(as.factor(clust$clust)))
#clust2temp$clust = rownames(clust2temp)
#colnames(clust2temp) = c("totalclust","clust")
#clust2temp$totalclust = paste("tot=",clust2temp$totalclust,sep="")
#clust2 = merge(clust2, clust2temp, by="clust")
#clust2$id2 = paste("(",clust2$realclust,";",clust2$length,";",clust2$totalclust,")",sep="")
#clust2$id3 = clust2$realclust
#clust2 = subset(clust2,select=c("clust","ymin","ymax","xpos0","xpos1","id2","id3"))
