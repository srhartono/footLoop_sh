...
sub parse_callFile {
	...
	my ($fileName1) = $input1 =~ /PEAKS_LOCAL\/(.+).local.bed$/; die "Can't parse fileName from input1=$LCY$input1$N\n\n" if not defined $fileName1;
	my $callFile = "$footPeak_folder/.CALL/$fileName1.out";
	$input1 = $callFile;
	my ($PCB, $BC, $plasmid, $desc, $gene) = $fileName1 =~ /(PCB\d+)_bcBC(\d+)_plasmid(.+)_desc(.+)_gene(.+)_(Pos|Neg)/;
	my $output = "$footPeak_folder/CONV/$fileName1.readconv";
