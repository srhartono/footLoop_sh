...
use vars qw($opt_v $opt_d $opt_n $opt_G $opt_c $opt_b);
getopts("vd:n:G:cb:");
...
my $breaks = defined $opt_b ? $opt_b : "250,500,750";
...
my %printlenclust;
$printlenclust{header} = "length_PEAK_" . join("\t", @breaks) . "\t>$breaks[@breaks-1]\tlength_PEAK_TEMP" . join("\t", @breaks) . "\t>$breaks[@breaks-1]";
foreach my $gene (sort keys %samFixedData) {
	my @key = qw(Pos Neg);
	my $include_cpg = $samData{uc($gene)}{include_cpg};
	my $goodstrand = $coor->{uc($gene)}{strand}; $goodstrand = "Pos" if $goodstrand eq "+"; $goodstrand = "Neg" if $goodstrand eq "-";
	my $tempstrand = $goodstrand eq "Pos" ? "Neg" : "Pos";
	my $goodconv = $goodstrand eq "Pos" ? "CH" : "GH";
	my $tempconv = $goodstrand eq "Pos" ? "GH" : "CH";
	my $goodconv_C = $goodstrand eq "Pos" ? "CG" : "GC";
	my $tempconv_C = $goodstrand eq "Pos" ? "GC" : "CG";
	my ($clustindiv_PEAK_C) = <$footPeakFolder/FOOTCLUST/CLUST_GENOME/*$goodstrand*$goodconv_C.PEAK.genome.bed.indiv.clust>;
	my ($clustindiv_PEAK) = <$footPeakFolder/FOOTCLUST/CLUST_GENOME/*$goodstrand*$goodconv.PEAK.genome.bed.indiv.clust>;
	my ($clustindiv_PEAK_TEMP_C) = <$footPeakFolder/FOOTCLUST/CLUST_GENOME/*$tempstrand*$tempconv_C.PEAK.genome.bed.indiv.clust>;
	my ($clustindiv_PEAK_TEMP) = <$footPeakFolder/FOOTCLUST/CLUST_GENOME/*$tempstrand*$tempconv.PEAK.genome.bed.indiv.clust>;
#	print "\n\n$YW DOING $gene$N\n\n$footPeakFolder/FOOTCLUST/CLUST_GENOME/*$goodstrand*$goodconv_C.PEAK.genome.bed.indiv.clust\n\n";
	$clustindiv_PEAK = join("\t", @{parse_clust($clustindiv_PEAK,$outLog)});
	$clustindiv_PEAK_C = join("\t", @{parse_clust($clustindiv_PEAK_C, $outLog)});
	$clustindiv_PEAK_TEMP = join("\t", @{parse_clust($clustindiv_PEAK_TEMP, $outLog)});
	$clustindiv_PEAK_TEMP_C = join("\t", @{parse_clust($clustindiv_PEAK_TEMP_C, $outLog)});
	$printlenclust{gene}{uc($gene)} = (defined $include_cpg and $include_cpg eq "TRUE") ? "$clustindiv_PEAK_C\t$clustindiv_PEAK_TEMP_C" : "$clustindiv_PEAK\t$clustindiv_PEAK_TEMP";
#	die;
...
sub parse_clust {
	...
	my @len = (0) x (@breaks + 1);
	my $total = 0;
	return \@len if not defined $input1;
	open (my $in, "<", $input1) or DIELOG($outLog, "Can't read from $input1: $!\n");
	while (my $line = <$in>) {
		chomp($line);
		my ($id, $beg, $end, $x, $xend, $clust, $clust2) = split("\t", $line);
		next if $clust2 =~ /,/;
		my $len = $end - $beg;
		$len[0] ++ if $len < $breaks[0];
		$len[@breaks] ++ if $len >= $breaks[@breaks-1] and @breaks-1 != 0;
		for (my $i = 1; $i < @breaks-1; $i++) {
			$len[$i] ++ if $len >= $breaks[$i-1] and $len < $breaks[$i];
		}
		$total ++;
	}
	return \@len if $total == 0;
	for (my $i = 0; $i < @len; $i++) {
		$len[$i] = int($len[$i]/$total * 1000+0.5)/1000;
	}
	print "Doing $YW$input1$N\n";
	print join("\t", @breaks) . "\t>$breaks[@breaks-1]\n" if $input1 =~ /Pos.+CG/;
	print join("\t", @len) . "\n" if $input1 =~ /Pos.+CG/;
	close $in;
	return \@len;
}
...
for (my $i = 0; $i < @types; $i++) { 
	$print{$types[$i]} = 0 if not defined $print{$types[$i]};
	print $out $print{$types[$i]};
	print $out2 "\t$types[$i]\t$types[$i]/total";
}
print $out2 "\t$printlenclust{header}\n";
...
foreach my $gene (sort keys %{$print0{gene}}) {
	...
	print $out2 "\t$printOTHERS\t$printOTHERSperc\t$printlenclust{gene}{uc($gene)}\n";
}
...
$LCY-b: Breaks [default: 250,500,750]$N
