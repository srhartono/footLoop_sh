...
my $readNameS  = getFilename($readFile);
...
my $samFileS   = $outDir .  "/$readNameS\_bismark_bt2.bam";
my $uuid       = getuuid();
my $date       = getDate();
...
sub fix_samFile {
	...
	if ($checkSam == 0) {
		...
		LOG($outLog, "\tgzip -f $origDir/$samFileName.fixed");
...
sub parse_samFile {
	...
	(open($sam, "samtools view $samFile|") and print "IS BAM\n") or LOG($outLog, "$LRD!!!$N\tFATAL ERROR: Could not open $samFile: $!") and exit 1 if $samFile =~ /.bam$/;
...
sub log_samFile {
	...
	#die "$origDir/$samFileName.fixed.gz\n";
...
sub run_bismark {
	...
	my $mybamtemp = $mybam;
#	if (not -e $mybamtemp) {
#		die "not exist $mybamtemp\n";
#		($mybamtemp) = <$outDir/*.bam>;
#		($mybamtemp) = <$outDir/*.sam> if not defined $mybamtemp;
#		if (defined $mybamtemp and -e $mybamtemp) {
#			system("/bin/ln -s $mybamtemp $mybam") == 0 or LOG($outLog, "Failed to /bin/ln -s $mybamtemp $mybam: $!\n");
#		}
#	}
	my $MAP = "";
	LOG($outLog, "\n\tb. Running bismark\n");
	$mybam =~ s/(.fq.gz|.fq|.fastq|.fastq.gz)_bismark_bt2/_bismark_bt2/;
	my $bamFile = $mybam; 
	my $ext = "bam";
	
	my ($mybamFilename) = getFilename($mybam);
	my $finalBamfile = $mybamFilename . "." . $ext;	
...
#	}
	if (defined $opt_F or not -e $mybam) {
		...
		if ($opt_p) {
			...
			for (my $p = 0; $p < @partSam; $p++) {
				...
				my $bismark_report_temp = $bismark_report;
				if (not -e $bismark_report_temp) {
					($bismark_report_temp) = <$outDir/*_report.txt>;
					system("/bin/ln -s $bismark_report_temp $bismark_report") == 0 or DIELOG($outLog, "Failed to ln -s $bismark_report_temp to $bismark_report: $!\n") if -e $bismark_report_temp;
				}
	...
	# print to $outReadLog
	LOG($outReadLog, "footLoop.pl,samFile,$outDir/$mybamFilename.$ext\n","NA");
	LOG($outReadLog, $MAP,"NA");

	return("$outDir/$mybamFilename.$ext");
}
