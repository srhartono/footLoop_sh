...
sub getFullpath {
    ...
    my ($folder, $filename);
    if ($file =~ /\//) {
        ($folder, $filename) = getFilename($file, "folderfull") if $file =~ /\//;
    }
    else {
        ($filename) = getFilename($file, "full");
        $folder = "./";
    }
    ...
    chdir $currdir;
    chomp($folder);
    #print "Folder = $folder, currdir = $currdir, File = $filename, pwd = $folder\n";
    #die;
...
#	print "Names=" . join(",", @names) . "\n\nFOLDER=$CY$folder$N, $shortname\n\n";
	...
	print "$0::getFilename: fh=$fh type=$type TYPE DOES NOT EXIST!, returning shortname=$LGN$shortname$N\n";
	return $shortname;
}
