...
LOG($outLog, date() . "Done checking, fixing sam/bam file!\n");
my %out;
...
sub check_file {
	...
	LOG($outLog, __LINE__ .  "\n\n$LRD----DEAD!----$N\n\nfootLoop_2fixsam.pl: linecount=$linecount file=$file type=$type, check=$check, check_file failed (does not seem to be a sam file\n\t-> there's no read with more than 10 collumns!\n") if $check == 0;
}
	...
	if (-e $footLoop_logFile) {
		...
		for (my $i = 0; $i < @line; $i++) {
			...
			if ($line =~ /footLoop.pl,samFile/) {
				...
				($samFile) = <$samFile*bam> if not -e $samFile;
			}
