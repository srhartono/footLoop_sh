...
use vars qw($opt_v $opt_m $opt_M $opt_i $opt_F $opt_o);
getopts("vM:m:i:F:o:");

my ($mem, $max_squeue, $ccsdir, $outputDir) = ($opt_m, $opt_M, $opt_F, $opt_o);
...
my $folderFull1 = getFullpath($folder1);
my $folderFull = defined $outputDir ? $outputDir : $folder1;
$folderFull = getFullpath($folderFull);
$folderFull =~ s/\/\.\//\//g;
$folderFull =~ s/\/+/\//g;
print "use=$user, $ENV{HOME}\n";
makedir("$folderFull/sbatch") if not -d "$folderFull/sbatch";
makedir("$folderFull/sbatch/0_CCS") if not -d "$folderFull/sbatch/0_CCS";
makedir("$folderFull/sbatch/1_LOG") if not -d "$folderFull/sbatch/1_LOG";
# <-- Log File

my $outLogFile = "$folderFull/$fileName1.footLoop_Sequel_ZMW_logFile.txt";
open (my $outLog, ">", "$outLogFile") or die "Failed to write to $LCY$outLogFile$N: $!\n";
LOG($outLog, "folderFull=$folderFull\n");
...
LOG($outLog, date() . ": Doing samtools view $input1, getting 100 zmws per run\n");
...
#			}
			$zmws =~ s/,$//;
			my $cmd0 = "module load smrtlink/8.0";
			my $cmd  = "time ccs --zmws $zmws --reportFile $folderFull/sbatch/1_LOG/$fileName1\_$total_zmw\.bam_report.txt --logFile $folderFull/sbatch/1_LOG/$fileName1\_$total_zmw\.bam_log.txt $folderFull1/$fileName1 $folderFull/sbatch/0_CCS/$fileName1\_$total_zmw\.bam_ccs.fq";
			my $out1;
			...
			print $out0 "#!/bin/bash -l\n#SBATCH -p high --mem $mem -t 999 --job-name long_sbatch_$total_zmw --output \%j_long_sbatch_$total_zmw.sbout\necho \"total_length=$total_lenz\"\necho \"$cmd0\"\necho \"$cmd\"\n$cmd0\n$cmd\n\n";
...
die;
		...
		if (keys %run != 0) {
			...
			$zmws =~ s/,$//;
			my $cmd0 = "module load smrtlink/8.0";
			my $cmd  = "time ccs --zmws $zmws --reportFile $folderFull/sbatch/1_LOG/$fileName1\_$total_zmw\.bam_report.txt --logFile $folderFull/sbatch/1_LOG/$fileName1\_$total_zmw\.bam_log.txt $folderFull/$fileName1 $folderFull/sbatch/0_CCS/$fileName1\_$total_zmw\.bam_ccs.fq";
			my $out1;
			open ($out1, ">", "$folderFull/sbatch/sbatch_$total_zmw\.sbatch") or die "Failed to write to $folderFull/sbatch/sbatch_$total_zmw\.sbatch: $!\n";
