...
use vars qw($opt_v $opt_d $opt_n $opt_G $opt_t $opt_R $opt_D $opt_L);
getopts("vd:n:G:t:RD:L");
...
my $toggleUseLen = defined $opt_L ? "No" : "Yes";
...
-L: Toggle to NOT use length cluster
";
...
library(Cairo)
   ...
   toggleUseLen = \"$toggleUseLen\"

	df\$len = df\$xend - df\$xbeg
	df\$lenclust = 1
	if (dim(df[df\$len >= 250,][1] != 0)) {
		df[df\$len >= 250,]\$lenclust = 2
	}
	if (dim(df[df\$len >= 500,][1] != 0)) {
		df[df\$len >= 500,]\$lenclust = 3
	}
	if (dim(df[df\$len >= 750,][1] != 0)) {
		df[df\$len >= 750,]\$lenclust = 4
	}
	if (Rstrand == \"Neg\" & toggleRstrand == \"Yes\") {
      if (toggleUseLen == \"Yes\") {
			df = df[order(-df\$lenclust,-df\$clust, -as.integer(df\$xend/$windowDiv), -as.integer(df\$xbeg/$windowDiv)),]
		} else {
			df = df[order(-df\$clust, -df\$lenclust, -as.integer(df\$xend/$windowDiv), -as.integer(df\$xbeg/$windowDiv)),]
		}
	} else {
      if (toggleUseLen == \"Yes\") {
			df = df[order(df\$lenclust,df\$clust, as.integer(df\$xbeg/$windowDiv), as.integer(df\$xend/$windowDiv)),]
		} else {
			df = df[order(df\$clust,df\$lenclust, as.integer(df\$xbeg/$windowDiv), as.integer(df\$xend/$windowDiv)),]
		}
	}
	# aggregate mean xbeg and xend position then sort cluster number
	df2 = as.data.frame(aggregate(subset(df,select=c(xbeg,xend)),by=list(df\$lenclust,df\$clust),function(x)mean(x,trim=0.05)))
	colnames(df2) = c(\"clust\",\"xbeg\",\"xend\")
	df2\$len = df2\$xend - df2\$xbeg
	df2\$lenclust = 1
	if (dim(df2[df2\$len >= 250,])[1] != 0)) {
		df2[df2\$len >= 250,]\$lenclust = 2
	}
	if (dim(df2[df2\$len >= 500,][1] != 0)) {
		df2[df2\$len >= 500,]\$lenclust = 3
	}
	if (dim(df2[df2\$len >= 750,][1] != 0)) {
		df2[df2\$len >= 750,]\$lenclust = 4
	}
	if (Rstrand == \"Neg\" & toggleRstrand == \"Yes\") {
      if (toggleUseLen == \"Yes\") {
			df2 = df2[order(-df2\$lenclust, -df2\$clust, -as.integer(df2\$xend/$windowDiv), -as.integer(df2\$xbeg/$windowDiv)),]
		} else {
			df2 = df2[order(-df2\$clust, -df2\$lenclust, -as.integer(df2\$xend/$windowDiv), -as.integer(df2\$xbeg/$windowDiv)),]
		}
	} else {
      if (toggleUseLen == \"Yes\") {
			df2 = df2[order(df2\$lenclust, df2\$clust, as.integer(df2\$xbeg/$windowDiv), as.integer(df2\$xend/$windowDiv)),]
		} else {
			df2 = df2[order(df2\$clust, df2\$lenclust, as.integer(df2\$xbeg/$windowDiv), as.integer(df2\$xend/$windowDiv)),]
		}
	}
	df2\$clust2 = seq(1,dim(df2)[1])
	df2 = subset(df2,select=c(-lenclust,-len))

	# merge cluster number to df
	df = as.data.frame(merge(df,subset(df2,select=c(clust,clust2) ) ))
	df\$clust = df\$clust2
	df\$id2 = paste(df\$id,\".\",df\$clust,sep=\"\")
	df = subset(df,select=c(-clust2,-len))

	mylen = $windowDiv;
	if (Rstrand == \"Neg\" & toggleRstrand == \"Yes\") {
      if (toggleUseLen == \"Yes\") {
			df = df[order(-df\$lenclust,-df\$clust, -as.integer((df\$xend-df\$xbeg)/250), -as.integer(df\$xend/mylen), -as.integer(df\$xbeg/mylen)),]
		} else {
			df = df[order(-df\$clust,-as.integer((df\$xend-df\$xbeg)/250), -df\$lenclust, -as.integer(df\$xend/mylen), -as.integer(df\$xbeg/mylen)),]
		}
	} else {
      if (toggleUseLen == \"Yes\") {
			df = df[order(df\$lenclust,df\$clust,as.integer((df\$xend-df\$xbeg)/250), as.integer(df\$xbeg/mylen), as.integer(df\$xend/mylen)),]
		} else {
			df = df[order(df\$clust,as.integer((df\$xend-df\$xbeg)/250), df\$lenclust, as.integer(df\$xbeg/mylen), as.integer(df\$xend/mylen)),]
		}
	}


	if (toggleUseLen == \"Yes\") {
		df=subset(df,select=-lenclust)
		alllenclust = unique(df\$lenclust)
		lasty = 0
		for (i in 1:length(alllenclust)) {
			temp = df[df\$lenclust == alllenclust[i],]
			temp\$ybeg = seq(1,dim(temp)[1]) + lasty
			temp\$yend = seq(2,dim(temp)[1]+1) + lasty
			lasty = dim(temp)[1]+1+lasty
			curr_last_cluster = max(temp\$clust)
			temp\$clust = temp\$clust + last_cluster
			last_cluster = curr_last_cluster
			temp = subset(temp,select=c(id,xbeg,xend,ybeg,yend,clust,id2));# temp\$h = h
			if (i == 1) {
				df.final = temp
			} else {
				df.final = rbind(df.final,temp)
			}
		}
		df = df.final
	} else {
	  mylenz = as.integer((df\$xend - df\$xbeg)/250);
	   df\$clust = (mylenz+1) * 10
...
#	df\$ybeg = seq(1,dim(df)[1])
#	df\$yend = seq(2,dim(df)[1]+1)
	png(type=\"cairo\",file=\"$tempFile2_PNG\",height=(dim(df)[1])*5,width=max(df\$xend)+10)
